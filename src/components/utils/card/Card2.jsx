...
export default function Card2({title, src, price, location,date, rating, id, status, reservationId}) {
  ...
  return (
    <>
      <section className='col-span-1 group border-2 rounded-xl shadow-lg shadow-black/15'>
        <div className="flex flex-col justify-center items-center gap-2 w-full h-max">
          ...
          <div className="flex flex-col p-2 relative w-full">
            <p className='flex items-center overflow-x-scroll text-nowrap [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] font-semibold text-lg'>{location} (<span className='text-sm text-center'>{title}</span>)</p>
      ...
      {toggle && <CancelLargeSection setToggle={setToggle} route='/api/cancelbooking' id={reservationId} />}
    </>
  )
}
